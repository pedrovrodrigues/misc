...
std::string sum3(std::string a, std::string b, std::string c) {
    a = ax_0s(a);
    b = ax_0s(b);
    c = ax_0s(c);
    int n = std::max({a.length(), b.length(), c.length()});
    std::reverse(a.begin(), a.end());
    std::reverse(b.begin(), b.end());
    ...
    int carry = 0;
    for (int i = 0; i < n; i++) {
        int s = (a[i] - '0') + (b[i] - '0') + (c[i] - '0') + carry;
        rd += std::to_string(s % 2);
        carry = s / 2;
    } 
    if (carry == 1) {
        rd += "10";
...
std::string mul3(std::string a, std::string b) {
    a = a.insert(0, 3 - a.length(), '0');
    b = b.insert(0, 3 - b.length(), '0');
    if (a == "000") {
        return "0";
    } else if (a == "001") {
        return b;
...
std::string mul(std::string a, std::string b) {
    ...
    std::string m1 = mul(a1, b1);
    std::string m2 = mul(a2, b2);
    std::string p1 = sum(a1, a2);
    std::string p2 = sum(b1, b2);
    std::string m3 = mul(p1, p2);
    std::string m4 = sub(m3, sum(m1, m2));
    m1 += std::string(n, '0');
    m4 += std::string(mid, '0');
